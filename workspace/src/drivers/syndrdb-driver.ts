...
import { SyndrDBElectronAPI } from './electron-api';
...
export class SyndrDBDriver {
  ...
  async connect(config: ConnectionConfig): Promise<boolean> {
    if (this.connected) {
      console.log('⚠️ Already connected to SyndrDB, skipping connection attempt');
      return true; // Return true if already connected instead of throwing
    }

    console.log('🚀 SyndrDBDriver: Starting connection process...');
    this.config = config;
      ...
      const statusHandler = (data: any) => {
        ...
        if (data.connectionId === this.connectionId) {
          ...
          if (data.status === 'connected') {
            ...
            resolved = true;
            clearTimeout(timeout);
            this.connected = false; // Reset connection state on error
            this.connectionId = null;
            resolve(false);
          } else if (data.status === 'disconnected') {
            console.log('🔌 Connection was disconnected unexpectedly');
